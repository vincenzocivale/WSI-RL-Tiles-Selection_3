...
class OpenSlideWSI(WSI):
    ...
    def _lazy_initialize(self) -> None:
        ...
        if not self.lazy_init:
            try:
                if not openslide.OpenSlide.detect_format(self.slide_path):
                    raise RuntimeError(f"Unsupported or missing image file (format not detected by OpenSlide): {self.slide_path}")
